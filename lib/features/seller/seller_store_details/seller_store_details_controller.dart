...
class SellerStoreDetailsController extends GetxController {
  ...
  getStoreInfo() async {
    try {
      ...
      final storeResult =
          await SellerStoreRepository.getStoreById(storeId: _storeId);
      if (storeResult == null) {
        CustomLoading.stop();
      ...
      final storeProductsResult =
          await SellerProductRepository.getStoreProductsById(storeId: _storeId);
  ...
  onNavigateAddNewProduct() {
    Get.off(() => const SellerAddNewProductToStoreScreen(),
        arguments: [_storeId, store.value.title]);
  }
                  ...
                  onClick: () async {
                    try {
                      ...
                      for (Product product in storeProducts) {
                        await SellerProductRepository.deleteImage(
                            imgName: "${product.id}_img1");
                        await SellerProductRepository.deleteImage(
                            imgName: "${product.id}_img2");
                        await SellerProductRepository.deleteImage(
                            imgName: "${product.id}_img3");
                        await SellerProductRepository.deleteProduct(
                            productId: product.id ?? "");
                      }
                      await SellerStoreRepository.deleteImage(
                          imgName: store.value.id ?? "");
                      await SellerStoreRepository.deleteStore(
                          storeId: _storeId);
